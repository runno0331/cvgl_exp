/*
法線ベクトルの意味
光が当たる確度を反映するのに必要。これがでたらめであると影による
陰影のつき方がでたらめになってしまう。
*/
/*
*/
...
void glut_display(){
    ...
    GLfloat lightpos2[] = {(float)(3 * cos(g_angle4) * sin(g_angle3/3)+2),
                            (float)(3 * sin(g_angle4/2)),
                            (float)(3 * cos(g_angle4) * cos(g_angle3/2)),
                            1.0};
    ...
    glTranslatef((float)(3 * cos(g_angle4) * sin(g_angle3/3)+2),
                (float)(3 * sin(g_angle4/2)),
                (float)(3 * cos(g_angle4) * cos(g_angle3/2)));
    glutSolidSphere(0.2, 50, 50);
    glPopMatrix();
